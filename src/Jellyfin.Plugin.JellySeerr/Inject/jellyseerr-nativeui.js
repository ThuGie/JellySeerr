    ...
    window.jellySeerrNativeUi = {
        ...
        buildRowSkeleton: function (plugin, title, kind) {
            ...
            const sectionClass = isCarousel
                ? 'verticalSection jellySeerr-carousel-section jellySeerr-skeleton-section'
                : 'verticalSection jellySeerr-poster-section jellySeerr-skeleton-section';
            ...
            for (let i = 0; i < count; i++) {
                const metaHtml = isCarousel ? '' : `
                    <div class="cardText cardTextCentered cardText-secondary">
                        <span class="jellySeerr-native-skeleton-text jellySeerr-native-skeleton-text--meta"></span>
                    </div>`;
                cards += `
                    <div class="card ${cardType} card-hoverable" data-jellySeerr-skeleton-card="true" aria-hidden="true">
                        <div class="cardBox cardBox-bottompadded">
                            <div class="cardScalable">
                                <div class="cardPadder ${padderType}"></div>
                                <div class="cardImageContainer coveredImage cardContent jellySeerr-native-skeleton-image" role="presentation"></div>
                            </div>
                            <div class="cardText cardTextCentered cardText-first">
                                <span class="jellySeerr-native-skeleton-text jellySeerr-native-skeleton-text--title"></span>
                            </div>
                            ${metaHtml}
                        </div>
                    </div>`;
            ...
            return plugin.mountFromHtml(`
                <div class="${sectionClass}" aria-hidden="true" data-jellySeerr-skeleton="true" data-jellySeerr-native-skeleton="true">
        ...
        createBoxCard: function (plugin, item, mediaType, kind) {
            ...
            return `
                <div class="card overflowBackdropCard card-hoverable"
                    data-jellySeerr-box-card="true" data-kind="${kind}" data-media-type="${mediaType}" data-id="${plugin.escapeHtml(String(id || ''))}" data-name="${safeName}" role="button" tabindex="0">
                    <div class="cardBox cardBox-bottompadded">
                        <div class="cardScalable">
        ...
        renderGridViewShell: function () {
            return `
                <div data-jellySeerr-grid-view="true" data-grid-shell="native">
                    <div class="verticalSection">
                        ...
                        <div class="flex align-items-center justify-content-center flex-wrap-wrap padded-left padded-right focuscontainer-x">
                            <div class="paging"><div class="listPaging"><span data-jellySeerr-paging-text style="vertical-align:middle;"></span></div></div>
                        ...
                        <div data-jellySeerr-loadmore style="display:none" class="paging padded-left padded-right">
                            <button type="button" class="raised emby-button">Load more</button>
                        </div>
                        <div data-jellySeerr-status style="display:none" class="listItemBodyText secondary padded-left padded-right"></div>
                    </div>
                </div>`;
        },
        ...
        getJellyfinWebpackRequire: function () {
            if (window.__jellySeerrWebpackRequire) {
                return window.__jellySeerrWebpackRequire;
            }
            ...
            try {
                let req = null;
                window.webpackChunk.push([[`jellySeerr_native_${Date.now()}`], {}, function (__webpackRequire) {
                    req = __webpackRequire;
                }]);
                window.__jellySeerrWebpackRequire = req;
                return req;
            } catch (err) {
        ...
        updateGridChrome: function (gridView) {
            ...
            const pagingText = gridView.querySelector('[data-jellySeerr-paging-text]');
            if (pagingText) {
