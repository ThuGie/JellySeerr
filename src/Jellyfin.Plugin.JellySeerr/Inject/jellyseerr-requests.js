...
(function () {
    if (window.__jellySeerrRequestsInit) {
        return;
    }
    window.__jellySeerrRequestsInit = true;
    ...
    function getPlugin() {
        return window.jellySeerrPlugin || null;
    }
    ...
    function findActiveContainer() {
        const all = document.querySelectorAll('.jellySeerr-requests-sections');
    ...
    function chipClassForStatus(label) {
        ...
        if (normalized.includes('available') && !normalized.includes('partially')) {
            return 'jellySeerr-request-chip--available';
        }
        if (normalized.includes('partially')) {
            return 'jellySeerr-request-chip--partial';
        }
        if (normalized.includes('processing')) {
            return 'jellySeerr-request-chip--processing';
        }
        if (normalized.includes('pending')) {
            return 'jellySeerr-request-chip--pending';
        }
        if (normalized.includes('approved')) {
            return 'jellySeerr-request-chip--approved';
        }
        if (normalized.includes('declined') || normalized.includes('failed')) {
            return 'jellySeerr-request-chip--declined';
        }
        return '';
    }
    ...
    function getAvatarUrl(avatarPath) {
        ...
        return ApiClient.getUrl('JellySeerr/proxy/avatar', {
            path: avatarPath,
    ...
    function renderContentBlock(item, isLandscape) {
        ...
        const avatarHtml = avatarUrl
            ? `<img class="jellySeerr-request-avatar" src="${escapeHtml(avatarUrl)}" alt="" loading="lazy" onerror="this.remove()">`
            : '';
        ...
        if (item.type) {
            chips.push(`<span class="jellySeerr-request-chip jellySeerr-request-chip--type">${escapeHtml(item.type === 'tv' ? 'TV' : 'Movie')}</span>`);
        }

        const yearHtml = item.year ? ` <span class="jellySeerr-request-year">(${escapeHtml(String(item.year))})</span>` : '';
        const timeAgoHtml = timeAgo ? ` &bull; <span>${timeAgo}</span>` : '';

        const seasonNumbers = Array.isArray(item.seasonNumbers) ? item.seasonNumbers : [];
        const seasonsHtml = item.type === 'tv' && seasonNumbers.length ? `<div class="jellySeerr-request-meta">Requested seasons: <span class="jellySeerr-request-meta-light">${escapeHtml(seasonNumbers.join(', '))}</span></div>` : '';

        const progress = getServarrProgress(item);
        const progressSpacerHtml = progress && !isLandscape ? '<div class="jellySeerr-request-progress-spacer"></div>' : '';
        const progressHtml = progress ? renderProgressBlock(item) : '';

        return `
            <div class="jellySeerr-request-content">
                <div class="jellySeerr-request-title">
                    <span title="${safeTitle}">${safeTitle}</span>${yearHtml}
                </div>
                <div class="jellySeerr-request-chips">${chips.join('')}</div>
                <div class="jellySeerr-request-meta">
                    <span>Requested by</span> ${avatarHtml}
                    <span class="jellySeerr-request-meta-light">${escapeHtml(item.requestedBy || 'Unknown')}</span>${timeAgoHtml}
                </div>
                ${seasonsHtml}
                ${progressSpacerHtml}
                ${progressHtml}
    ...
    function renderRequestCard(item) {
        ...
        const layoutClass = isLandscape ? 'jellySeerr-request-box--landscape' : 'jellySeerr-request-box--portrait';
    ...
    function matchesFilter(item, filterId) {
        ...
        if (filterId === 'failed') {
            return label === 'failed' || item.isFailed === true;
        }
    ...
    function renderPagination(page, totalPages) {
        return `
            <button type="button" class="raised emby-button jellySeerr-requests-prev" ${page <= 1 ? 'disabled' : ''}>Previous</button>
            <span class="jellySeerr-requests-page-info">Page ${page} of ${totalPages}</span>
            <button type="button" class="raised emby-button jellySeerr-requests-next" ${page >= totalPages ? 'disabled' : ''}>Next</button>`;
    }

    function updatePagination(container, totalPages) {
        let pagination = container.querySelector('.jellySeerr-requests-pagination');
        ...
        if (!pagination) {
            ...
            pagination.className = 'jellySeerr-grid-loadmore jellySeerr-requests-pagination padded-left';
    ...
    function renderPage(container) {
        ...
        const body = container.querySelector('.jellySeerr-requests-body');
        if (!body) {
            return;
        }

        if (!state.pageRequests.length) {
            body.innerHTML = `<div class="jellySeerr-empty-row padded-left">No requests found.</div>`;
        ...
        const gridClass = shouldUseLandscapeCards()
            ? ' jellySeerr-requests-grid--landscape'
            : ' jellySeerr-requests-grid--portrait';

        body.innerHTML = `
            <div class="jellySeerr-requests-grid${gridClass} padded-left padded-right">${cards}</div>`;
    ...
    function setReloadButtonState(container, mode) {
        const reloadBtn = container && container.querySelector('.jellySeerr-requests-reload');
        if (!reloadBtn) {
            return;
        }
    ...
    function loadRequests(container, options) {
        ...
        if (body && !keepVisible) {
            body.innerHTML = `<div class="jellySeerr-loading-row padded-left">Loading...</div>`;
        }
        ...
        fetchRequestsPage()
            ...
            .catch(function (err) {
                ...
                if (body) {
                    body.innerHTML = `<div class="jellySeerr-empty-row padded-left">Could not load requests. Check Seerr settings.</div>`;
                }
                setReloadButtonState(container, 'idle');
            })
            .then(function () {
    ...
    function refreshIfActive(options) {
        ...
        if (!container || !container.querySelector('.jellySeerr-requests-panel')) {
            return;
        }
    ...
    function renderRequestsPanel() {
        const filterButtons = FILTERS.map(function (filter) {
            ...
            return `
                <button type="button" class="jellySeerr-requests-filter${activeClass}" data-filter="${filter.id}">
        ...
        return `
            ...
            <div class="jellySeerr-requests-body"></div>`;
    }

    function mount(container) {
        if (container.querySelector('.jellySeerr-requests-panel')) {
            return;
        }
    ...
    function bindContainerEvents(container) {
        if (container.dataset.jellySeerrRequestsBound === 'true') {
            return;
        }
        ...
        container.addEventListener('click', function (event) {
            ...
            const tab = event.target.closest('.jellySeerr-requests-filter');
            if (tab) {
                event.preventDefault();
            ...
            if (event.target.closest('.jellySeerr-requests-prev') && state.page > 1) {
                state.page -= 1;
                loadRequests(container);
                return;
            }

            if (event.target.closest('.jellySeerr-requests-next') && state.page < state.totalPages) {
                state.page += 1;
                loadRequests(container);
            }
        });
    ...
    function ensureMounted(options) {
        ...
        const alreadyMounted = !!container.querySelector('.jellySeerr-requests-panel');
        if (!alreadyMounted) {
            mount(container);
            return;
        }
    ...
    function init() {
        ...
        window.__jellySeerrRequestsEnsureMounted = ensureMounted;
