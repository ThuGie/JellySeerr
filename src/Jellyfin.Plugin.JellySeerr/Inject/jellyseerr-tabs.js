...
if (typeof window.jellySeerrPlugin === 'undefined') {
    const log = window.jellySeerrLog;

    window.jellySeerrPlugin = {
        movieRows: [
        ...
        TAB_DEFS: {
            ...
            tv: { sectionClass: 'jellySeerr-tv-sections', defaultTitle: 'TV Shows' },
            requests: { sectionClass: 'jellySeerr-requests-sections', defaultTitle: 'Requests' }
        },
        ...
        cleanupjellySeerrHeaderButtons: function () {
            document.querySelectorAll('.headerTabs [data-jellySeerr-tab]').forEach(function (btn) {
                btn.remove();
            });
        },

        jellySeerrBarKey: function (id) {
            return 'js:' + String(id || '').toLowerCase();
        },
        ...
        isLegacyjellySeerrCustomTabHtml: function (html) {
        ...
        normalizeBarOrder: function (barOrder, customTabs) {
            ...
            (Array.isArray(barOrder) ? barOrder : []).forEach(function (raw) {
                ...
                if (key.indexOf('ct:') === 0) {
                    ...
                    if (!isNaN(index) && index >= 0 && index < customList.length && !customList[index].legacyjellySeerr) {
        ...
        loadCustomTabsConfig: function () {
            ...
            function mapTabs(tabs) {
                return (Array.isArray(tabs) ? tabs : []).map(function (tab, index) {
                    return {
                        index: index,
                        legacyjellySeerr: self.isLegacyjellySeerrCustomTabHtml(tab.ContentHtml || tab.contentHtml || '')
                    };
                });
            }
        ...
        attachPluginTabGuard: function (tabs) {
            if (!tabs || tabs.dataset.jellySeerrPluginTabGuard === 'true') {
                return;
            }

            tabs.dataset.jellySeerrPluginTabGuard = 'true';
            ...
            tabs.addEventListener('beforetabchange', function (event) {
                ...
                // sync by data-index so jellySeerr panels stay in the right order.
                self.syncTabPanelsActiveState(index);

                const selectedButton = tabs.querySelector('.emby-tab-button[data-index="' + index + '"]');
                if (selectedButton && selectedButton.getAttribute('data-jellySeerr-tab')) {
                    log.info('jellySeerr tab before change to index ' + index);
                    setTimeout(function () {
                        if (self.isHomeTabContext()) {
                            self.onjellySeerrTabShown();
                        }
                    }, 0);
                }
            }, true);
            ...
            tabs.addEventListener('tabchange', function (event) {
                ...
                const isPluginTab = !!selectedButton.getAttribute('data-jellySeerr-tab') ||
        ...
        onjellySeerrTabShown: function () {
            const self = this;
            log.info('jellySeerr tab shown, scheduling render');
            setTimeout(function () {
                self.scheduleRender();
                if (typeof window.__jellySeerrRequestsEnsureMounted === 'function') {
                    window.__jellySeerrRequestsEnsureMounted({ tabShown: true });
                }
                if (typeof window.__jellySeerrLetterboxdEnsureMounted === 'function') {
                    window.__jellySeerrLetterboxdEnsureMounted();
                }
            }, 0);
        },

        removeObsoletejellySeerrPanels: function (page, slots) {
            const tabDefs = this.TAB_DEFS;
            ...
            const activeIds = new Set(slots.filter(function (slot) {
                return slot.type === 'jellySeerr';
            }).map(function (slot) {
                return slot.id;
            }));

            page.querySelectorAll('.tabContent').forEach(function (panel) {
                const id = panel.getAttribute('data-jellySeerr-tab');
                if (id) {
                    if (!activeIds.has(id)) {
                        panel.remove();
                    }
        ...
        createTabButton: function (id, title) {
            ...
            button.setAttribute('data-jellySeerr-tab', id);
        ...
        applyTabTitles: function (tabsSlider, slots) {
            ...
            (slots || []).forEach(function (slot) {
                if (slot.type !== 'jellySeerr') {
                    return;
                }
                const button = tabsSlider.querySelector('[data-jellySeerr-tab="' + slot.id + '"]');
        ...
        createTabPanel: function (id) {
            ...
            panel.setAttribute('data-jellySeerr-tab', id);
        ...
        buildDesiredBarSlots: function (config) {
            ...
            return (config.tabBarOrder || []).map(function (key) {
                ...
                if (key.indexOf('js:') === 0) {
                    ...
                    return { key: key, type: 'jellySeerr', id: id, title: tab.title };
                }
                if (key.indexOf('ct:') === 0) {
                    const index = parseInt(key.slice(3), 10);
                    const custom = (config.customTabs || [])[index];
                    if (!custom || custom.legacyjellySeerr) {
                        return null;
                    }
        ...
        applyBarOrder: function (page, tabsSlider, slots) {
            ...
            for (let i = 0; i < slots.length; i++) {
                ...
                } else if (slot.type === 'jellySeerr') {
                    button = tabsSlider.querySelector('[data-jellySeerr-tab="' + slot.id + '"]');
                    panel = page.querySelector('.tabContent[data-jellySeerr-tab="' + slot.id + '"]');
                }
            ...
            self.removeObsoletejellySeerrPanels(page, slots);
            ...
            planned.forEach(function (item, placedIndex) {
                ...
                if (item.slot.type === 'jellySeerr') {
                    if (!(button && button.getAttribute('data-jellySeerr-tab') === item.slot.id)) {
                        button = self.createTabButton(item.slot.id, item.slot.title);
                    } else {
                        self.setTabButtonTitle(button, item.slot.id, item.slot.title);
                    }
                    if (!(panel && panel.getAttribute('data-jellySeerr-tab') === item.slot.id)) {
            ...
            // Put Jellyfin, Custom Tabs, and jellySeerr panels in bar order.
            ...
            self._tabsEnsuring = self.loadTabConfig().then(function (config) {
                ...
                return self.waitForCustomTabs(config.customTabs, 20).then(function () {
                    ...
                    const panelsMatch = slots.every(function (slot) {
                        if (slot.type === 'jellySeerr') {
                            return !!page.querySelector('.tabContent[data-jellySeerr-tab="' + slot.id + '"]');
                        }
        ...
        isContainerLoading: function (container) {
            return container.dataset.jellySeerrLoading === 'true';
        },

        isContainerPopulated: function (container) {
            return container.querySelector('.jellySeerr-poster-section') !== null;
        },
        ...
        isGridViewOpen: function (container) {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                if (child.hasAttribute('data-jellySeerr-grid-view') && child.style.display !== 'none') {
                    return true;
                }
            }
            return false;
        ...
        cancelContainerLoad: function (container) {
            ...
            container.dataset.jellySeerrLoadId = 'cancelled-' + Date.now() + '-' + Math.random().toString(16).slice(2);
            container.dataset.jellySeerrLoading = 'false';
            if (!this.isContainerPopulated(container)) {
                delete container.dataset.jellySeerrLoaded;
            }

            Array.from(container.childNodes).forEach(function (node) {
                if (node.nodeType === Node.COMMENT_NODE && /jellySeerr-(row|carousel)-slot/.test(node.nodeValue || '')) {
                    node.remove();
                }
            });
        },
        ...
        renderIfContainerVisible: function (type) {
            const selector = type === 'movies' ? '.jellySeerr-movies-sections' : '.jellySeerr-tv-sections';
            ...
            this.loadDisplaySettings().then(function () {
                ...
                if (container.dataset.jellySeerrDisplaySettings !== settingsKey) {
                    log.info(type + ' tab display settings changed, clearing container');
                    container.dataset.jellySeerrDisplaySettings = settingsKey;
                    if (self.isContainerPopulated(container)) {
                        container.innerHTML = '';
                        delete container.dataset.jellySeerrLoaded;
                        delete container.dataset.jellySeerrLoading;
                    }
                }
                ...
                const hasError = container.querySelector('.jellySeerr-empty-row');
                if (hasError && container.dataset.jellySeerrLoaded === 'true') {
                    return;
                }
        ...
        loadTab: function (type, container) {
            if (!container) {
                container = this.findActiveContainer(
                    type === 'movies' ? '.jellySeerr-movies-sections' : '.jellySeerr-tv-sections'
                );
            }
            ...
            container.dataset.jellySeerrLoading = 'true';
            container.dataset.jellySeerrLoadId = loadId;
            delete container.dataset.jellySeerrLoaded;

            // Ignore results if user switched tabs or theres a newer load.
            const finishLoading = function () {
                if (container.dataset.jellySeerrLoadId !== loadId) {
                    log.info(type + ' tab load finished but was superseded');
                    return;
                }
                container.dataset.jellySeerrLoading = 'false';
                container.dataset.jellySeerrLoaded = 'true';
                log.info(type + ' tab load complete');
            };

            const isStale = function () {
                return container.dataset.jellySeerrLoadId !== loadId || !self.isContainerVisible(container) || self.isGridViewOpen(container);
            };
            ...
            self.loadDisplaySettings().then(function () {
                ...
                const checkAllDone = function () {
                    ...
                    if (!anySuccess && !isStale()) {
                        ...
                        container.innerHTML = `<div class="jellySeerr-empty-row">Failed to load discovery rows. Check Seerr settings and that your Jellyfin user is linked in Seerr.</div>`;
                    }
                    finishLoading();
                };
                ...
                const replaceSkeleton = function (skeleton, node) {
                    ...
                    if (node) {
                        node.classList.add('jellySeerr-section-fadein');
        ...
        fetchDiscover: function (path, query) {
            let url = ApiClient.getUrl('JellySeerr/' + path);
            if (query) {
                url += query;
            }
            return ApiClient.ajax({
        ...
        fetchJson: function (path) {
            return ApiClient.ajax({
                url: ApiClient.getUrl('JellySeerr/' + path),
                type: 'GET',
                dataType: 'json'
            });
        },
        ...
        loadDisplaySettings: function () {
            ...
            self._displaySettingsPromise = ApiClient.ajax({
                url: ApiClient.getUrl('JellySeerr/display-settings') + '?_=' + Date.now(),
                type: 'GET',
                dataType: 'json',
                cache: false
            }).then(function (data) {
        ...
        syncElegantFinFixes: function () {
            ...
            document.documentElement.classList.toggle('jellySeerr-elegantfin', enabled);
        },
        ...
        isNativeUiAvailable: function () {
            return !!(window.jellySeerrNativeUi &&
                typeof window.jellySeerrNativeUi.createDiscoverCards === 'function' &&
                typeof window.jellySeerrNativeUi.createBoxCard === 'function' &&
                typeof window.jellySeerrNativeUi.renderGridViewShell === 'function');
        },
        ...
        invalidateDisplaySettings: function () {
            ...
            document.querySelectorAll('.jellySeerr-movies-sections, .jellySeerr-tv-sections, .jellySeerr-search-section').forEach(function (section) {
                delete section.dataset.jellySeerrDisplaySettings;
            });
        ...
        renderLetterboxdCardChrome: function () {
            return `
                <span class="jellySeerr-letterboxd-select-indicator" aria-hidden="true">
        ...
        wrapDiscoverCard: function (cardHtml, options) {
            ...
            return `<div class="jellySeerr-letterboxd-card-slot">${cardHtml}</div>`;
        },
        ...
        buildRowSkeleton: function (title, kind, options) {
            ...
            if (useNative &&
                window.jellySeerrNativeUi &&
                typeof window.jellySeerrNativeUi.buildRowSkeleton === 'function') {
                return window.jellySeerrNativeUi.buildRowSkeleton(this, title, kind);
            }

            const useBackdrop = kind === 'poster' && this.shouldUseBackdropThumbnails();
            const cardClass = kind === 'carousel'
                ? 'jellySeerr-skeleton-card jellySeerr-skeleton-card--carousel'
                : `jellySeerr-skeleton-card jellySeerr-skeleton-card--poster${useBackdrop ? ' jellySeerr-skeleton-card--backdrop' : ''}`;
            ...
            return this.mountFromHtml(`
                <div class="verticalSection jellySeerr-skeleton-section" aria-hidden="true" data-jellySeerr-skeleton="true">
                    <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                        <div class="jellySeerr-skeleton-title" aria-label="${this.escapeHtml(title || 'Loading')}"></div>
                    </div>
                    <div class="jellySeerr-skeleton-track padded-left">${cards}</div>
                </div>`);
        },
        ...
        buildPosterRow: function (title, items, path, options) {
            ...
            if (!items || items.length === 0) {
                return this.mountFromHtml(`
                    <div class="verticalSection jellySeerr-poster-section">
                        <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                            <h2 class="sectionTitle sectionTitle-cards">${safeTitle}</h2>
                        </div>
                        <div class="jellySeerr-empty-row padded-left">No items to show</div>
                    </div>`);
            }

            const viewMoreBtn = path && items.length > 0 ? `
                <button type="button" class="jellySeerr-view-more" data-path="${this.escapeHtml(path)}" data-title="${safeTitle}">
                    View more \u2192
                </button>` : '';
            ...
            const section = this.mountFromHtml(`
                <div class="verticalSection jellySeerr-poster-section"${pathAttrs}>
                    <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                        <h2 class="sectionTitle sectionTitle-cards">${safeTitle}</h2>
                        ${viewMoreBtn}
                    </div>
        ...
        bindRowInfiniteScroll: function (section) {
            ...
            if (section.dataset.jellySeerrRowScrollBound === 'true') {
                return;
            }
            ...
            if (!scroller.scroller || typeof scroller.addScrollEventListener !== 'function') {
                const retries = parseInt(section.dataset.jellySeerrRowScrollRetries || '0', 10);
                const maxRetries = this._rowScrollBindRetries || 10;
                if (retries >= maxRetries) {
                    return;
                }
                section.dataset.jellySeerrRowScrollRetries = String(retries + 1);
                const self = this;
                requestAnimationFrame(function () {
            ...
            delete section.dataset.jellySeerrRowScrollRetries;
            section.dataset.jellySeerrRowScrollBound = 'true';
            section._jellySeerrRowScroller = scroller;
        ...
        scheduleRowScrollCheck: function (section) {
            if (section._jellySeerrRowScrollRaf) {
                return;
            }
            const self = this;
            section._jellySeerrRowScrollRaf = requestAnimationFrame(function () {
                section._jellySeerrRowScrollRaf = 0;
        ...
        checkRowScrollEnd: function (section) {
            ...
            const scroller = section._jellySeerrRowScroller;
        ...
        buildCarouselSection: function (title, items, mediaType, kind) {
            ...
            const cardsHtml = (items || []).map(function (item) {
                return useNativeCards
                    ? window.jellySeerrNativeUi.createBoxCard(self, item, mediaType, kind)
                    : self.createBoxCard(item, mediaType, kind);
            }).join('');

            if (!cardsHtml) {
                return this.mountFromHtml(`
                    <div class="verticalSection jellySeerr-carousel-section">
                        <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                            <h2 class="sectionTitle sectionTitle-cards">${safeTitle}</h2>
                        </div>
                        <div class="jellySeerr-empty-row padded-left">No items to show</div>
        ...
        createBoxCard: function (item, mediaType, kind) {
            ...
            let extraClass = ' jellySeerr-discover-card--fallback';
            let mediaHtml = '';
            ...
            if (kind === 'genre' && (this._displaySettings || {}).GenreUseBackdrops !== false) {
                ...
                if (backdrops.length) {
                    ...
                    if (backdropUrl) {
                        mediaHtml = `<span class="jellySeerr-discover-backdrop-media" style="background-image: url('${this.escapeHtml(backdropUrl)}')"></span>`;
                    }
                }
            }

            if (logoUrl) {
                extraClass = ' jellySeerr-box-card--logo';
                const logoClass = item.weirdSize ? 'jellySeerr-box-logo-weird' : 'jellySeerr-box-logo';
                mediaHtml = `<img class="${logoClass}" src="${this.escapeHtml(logoUrl)}" alt="${safeName}" loading="lazy" />`;
            } else {
                overlayTitleHtml = `<span class="jellySeerr-discover-overlay-title jellySeerr-box-label">${safeName}</span>`;
            }

            return `
                <div class="card jellySeerr-discover-card jellySeerr-discover-card--backdrop jellySeerr-box-card jellySeerr-discover-card--static${extraClass}"
                    data-jellySeerr-box-card="true" data-kind="${kind}" data-media-type="${mediaType}" data-id="${id}" data-name="${safeName}" role="button" tabindex="0">
                    <div class="cardBox">
                        <div class="cardScalable jellySeerr-discover-backdrop-scalable">
                            <div class="cardPadder jellySeerr-discover-backdrop-padder"></div>
                            <div class="cardImageContainer coveredImage cardContent jellySeerr-discover-backdrop-image" aria-label="${safeName}">
        ...
        createDiscoverCards: function (items, forGrid, options) {
            ...
            if (options.nativeCards === true) {
                return window.jellySeerrNativeUi.createDiscoverCards(this, items, Object.assign({}, options, {
        ...
        createDiscoverPosterCards: function (items, forGrid, options) {
            ...
            const staticClass = interactive ? '' : ' jellySeerr-discover-card--static';
            ...
            return items.map(function (item) {
                ...
                const letterboxdClass = letterboxdSlot
                    ? ` jellySeerr-letterboxd-card${selected ? ' is-selected' : ''}`
                    : '';
        ...
        createDiscoverBackdropCards: function (items, forGrid, options) {
            ...
            const interactive = options.interactive !== false;
            const includeMetaText = options.includeMetaText !== false;
            const gridClass = forGrid ? ' jellySeerr-discover-card--grid' : '';
            const staticClass = interactive ? '' : ' jellySeerr-discover-card--static';
            const boxClass = includeMetaText ? 'cardBox cardBox-bottompadded' : 'cardBox';
            const letterboxdSlot = options.letterboxdSlot === true;
            ...
            return items.map(function (item) {
                ...
                const metaHtml = includeMetaText ? (function () {
                    const meta = self.buildDiscoverYearText(item);
                    return `
                        <div class="cardText cardTextCentered cardText-first jellySeerr-discover-title-below"><bdi><span title="${safeName}">${safeName}</span></bdi></div>
                        <div class="cardText cardTextCentered cardText-secondary"><bdi><span title="${meta.year}">${meta.yearText}</span></bdi></div>`;
                })() : '';
                const selected = letterboxdSlot && typeof options.getLetterboxdSelected === 'function'
                    ? options.getLetterboxdSelected(parseInt(mediaId, 10))
                    : false;
                const letterboxdClass = letterboxdSlot ? ` jellySeerr-letterboxd-card${selected ? ' is-selected' : ''}` : '';
                    ...
                    <div class="card jellySeerr-discover-card jellySeerr-discover-card--backdrop${gridClass}${staticClass}${letterboxdClass}" data-tmdb-id="${mediaId}" data-media-type="${mediaType}"${fallbackAttr}${backdropPathAttr}${ariaSelected}>
                        <div class="${boxClass}">
                            <div class="cardScalable jellySeerr-discover-backdrop-scalable">
                                <div class="cardPadder jellySeerr-discover-backdrop-padder"></div>
                                <div class="cardImageContainer coveredImage cardContent jellySeerr-discover-backdrop-image" aria-label="${safeName}">
                                    <span class="jellySeerr-discover-backdrop-media"></span>
                                    <span class="jellySeerr-discover-overlay-title jellySeerr-box-label">${safeName}</span>
                                </div>
                                ${overlayHtml}
                                ${chromeHtml}
                            </div>
        ...
        setDiscoverBackdropImage: function (card, cachedUrl) {
            ...
            const media = card.querySelector('.jellySeerr-discover-backdrop-media');
            if (!media) {
                return;
            }
        ...
        setDiscoverBackdropPresentation: function (card, mode) {
            ...
            card.classList.remove(
                'jellySeerr-discover-card--english',
                'jellySeerr-discover-card--fallback'
            );

            if (mode === 'english') {
                card.classList.add('jellySeerr-discover-card--english');
                return;
            }

            if (mode === 'fallback') {
                card.classList.add('jellySeerr-discover-card--fallback');
            }
        },
        ...
        fetchTmdbBackdropBatch: function (items) {
            ...
            const promise = ApiClient.ajax({
                url: ApiClient.getUrl('JellySeerr/backdrops'),
                type: 'POST',
        ...
        applyDiscoverBackdropResult: function (card, result, seerrFallbackUrl) {
            ...
            const alreadyEnglish = card.classList.contains('jellySeerr-discover-card--english');
        ...
        hydrateDiscoverBackdropCards: function (container) {
            ...
            const cards = container.querySelectorAll('.jellySeerr-discover-card--backdrop:not([data-backdrop-hydrate-queued]):not([data-backdrop-loaded])');
            if (!cards.length) {
                return;
            }
            ...
            self.fetchTmdbBackdropBatch(batchItems).then(function () {
                ...
                cardEntries.forEach(function (entry) {
                    ...
                    if (entry.card.classList.contains('jellySeerr-discover-card--english') &&
        ...
        clearCardThumbSkeleton: function (root) {
            ...
            card.querySelectorAll('.jellySeerr-card-thumb-skeleton').forEach(function (skeleton) {
                if (skeleton.classList.contains('cardPadder')) {
                    skeleton.classList.remove('jellySeerr-card-thumb-skeleton');
                    return;
                }
                skeleton.remove();
            });
        ...
        hydrateNativeBackdropCards: function (container) {
            ...
            const cards = container.querySelectorAll('[data-jellySeerr-native-card="true"]:not([data-native-backdrop-hydrate-queued]):not([data-native-backdrop-loaded])');
            if (!cards.length) {
                return;
            }
        ...
        bindRequestHandler: function () {
            ...
            document.addEventListener('click', function (e) {
                ...
                if (!btn || !btn.closest('.jellySeerr-movies-sections, .jellySeerr-tv-sections, [data-jellySeerr-grid-view], .jellySeerr-search-section')) {
                    return;
                }
        ...
        bindCardClickHandler: function () {
            ...
            document.addEventListener('click', function (e) {
                ...
                const card = e.target.closest('.jellySeerr-discover-card, [data-jellySeerr-native-card="true"]');
                if (!card || card.classList.contains('jellySeerr-discover-card--static') ||
                    !card.closest('.jellySeerr-movies-sections, .jellySeerr-tv-sections, [data-jellySeerr-grid-view], .jellySeerr-search-section')) {
                    return;
                }
                ...
                if (window.jellySeerrModal && window.jellySeerrModal.open) {
        ...
        bindViewMoreHandler: function () {
            ...
            document.addEventListener('contextmenu', function (e) {
                if (e.target.closest('[data-jellySeerr-grid-view]')) {
                    e.preventDefault();
                    e.stopPropagation();
                }
            }, true);
            ...
            document.addEventListener('click', function (e) {
                const btn = e.target.closest('.jellySeerr-view-more');
                if (btn) {
                    const container = btn.closest('.jellySeerr-movies-sections, .jellySeerr-tv-sections');
                    if (!container) {
                        return;
                    }
                ...
                const boxCard = e.target.closest('[data-jellySeerr-box-card="true"]');
                if (!boxCard) {
                    return;
                }

                const boxContainer = boxCard.closest('.jellySeerr-movies-sections, .jellySeerr-tv-sections');
                if (!boxContainer) {
                    return;
                }
        ...
        renderLegacyGridViewShell: function () {
            return `
                <div class="jellySeerr-grid-view" data-jellySeerr-grid-view="true" data-grid-shell="legacy">
                    <div class="jellySeerr-grid-header padded-left padded-right">
                        <button type="button" class="jellySeerr-grid-back paper-icon-button-light emby-button" title="Back" aria-label="Back" data-grid-nav="back">
                            <span class="material-icons" aria-hidden="true">arrow_back</span>
                        </button>
                        <h2 class="jellySeerr-grid-title sectionTitle sectionTitle-cards"></h2>
                    </div>
                    <div>
                    ...
                    <div data-jellySeerr-loadmore style="display:none" class="jellySeerr-grid-loadmore">
                        <button type="button" class="raised emby-button">Load more</button>
                    </div>
                    <div data-jellySeerr-status style="display:none" class="jellySeerr-grid-status"></div>
                </div>`;
        },

        renderGridViewShell: function (useNativeGrid) {
            return useNativeGrid
                ? window.jellySeerrNativeUi.renderGridViewShell()
        ...
        applyGridViewMode: function (gridView) {
            ...
            const cards = gridView.querySelectorAll('[data-jellySeerr-native-card="true"]');
        ...
        openGridView: function (container, title, path) {
            ...
            Array.from(container.children).forEach(function (child) {
                if (!child.hasAttribute('data-jellySeerr-grid-view')) {
                    child.style.display = 'none';
                    child.dataset.jellySeerrHidden = 'true';
                }
            });

            const useNativeGrid = self.shouldUseNativeGridPages();
            const shellMode = useNativeGrid ? 'native' : 'legacy';
            let gridView = container.querySelector('[data-jellySeerr-grid-view]');
            ...
            if (!gridView) {
                ...
                gridView.querySelector('[data-jellySeerr-loadmore] button').addEventListener('click', function () {
                    self.loadMoreGridItems(gridView);
                });
            }

            if (useNativeGrid) {
                window.jellySeerrNativeUi.bindGridControls(self, gridView);
            }
            ...
            gridView.querySelector('[data-jellySeerr-loadmore]').style.display = 'none';
            gridView.querySelector('[data-jellySeerr-status]').style.display = 'none';
            gridView.style.display = '';
            if (useNativeGrid) {
                window.jellySeerrNativeUi.setGridControlsEnabled(gridView, false);
                window.jellySeerrNativeUi.updateGridChrome(gridView);
            }
        ...
        closeGridView: function (container) {
            ...
            const gridView = container.querySelector('[data-jellySeerr-grid-view]');
            if (gridView) {
            ...
            Array.from(container.children).forEach(function (child) {
                if (child.dataset.jellySeerrHidden === 'true') {
                    child.style.display = '';
                    delete child.dataset.jellySeerrHidden;
                }
            });
            this.scheduleRender();
        },
        ...
        loadMoreGridItems: function (gridView, isInitial) {
            ...
            const itemsContainer = gridView.querySelector('.itemsContainer');
            const loadMore = gridView.querySelector('[data-jellySeerr-loadmore]');
            const loadMoreBtn = loadMore.querySelector('button');
            const status = gridView.querySelector('[data-jellySeerr-status]');
            ...
            self.fetchDiscover(path, '?startIndex=' + encodeURIComponent(loadedCount) + '&limit=' + encodeURIComponent(pageSize)).then(function (result) {
                ...
                if (!result.items.length && loadedCount === 0) {
                    itemsContainer.innerHTML = `<div class="jellySeerr-empty-row">No items to show</div>`;
                ...
                if (useNativeCards) {
                    ...
                    window.jellySeerrNativeUi.setGridControlsEnabled(gridView, newCount > 0);
                    window.jellySeerrNativeUi.updateGridChrome(gridView);
                }
            }).catch(function (err) {
                if (!gridView.isConnected || gridView.dataset.gridSessionId !== gridSessionId || gridView.dataset.path !== path) {
                ...
                if (loadedCount === 0) {
                    itemsContainer.innerHTML = `<div class="jellySeerr-empty-row">Failed to load items.</div>`;
                }
                if (useNativeCards) {
                    window.jellySeerrNativeUi.updateGridChrome(gridView);
                }
        ...
        attachJellyfinSearchInput: function () {
            ...
            if (!input || input.dataset.jellySeerrSearchListener === 'true') {
                return;
            }

            input.dataset.jellySeerrSearchListener = 'true';
        ...
        removeSearchSection: function () {
            document.querySelectorAll('.jellySeerr-search-section').forEach(function (section) {
                section.remove();
            });
        },
        ...
        showSearchSkeleton: function (searchPage) {
            ...
            skeleton.classList.add('jellySeerr-search-section');
        ...
        fetchSeerrSearch: function (query) {
            ...
            const url = ApiClient.getUrl('JellySeerr/search') +
        ...
        runJellyfinSearch: function (query) {
            ...
            if (self._lastSearchQuery === query && document.querySelector('.jellySeerr-search-section')) {
                return;
            }
        ...
        refreshScrollers: function (container) {
            ...
            requestAnimationFrame(function () {
                requestAnimationFrame(function () {
                    ...
                    container.querySelectorAll('.jellySeerr-poster-section').forEach(function (section) {
                        self.bindRowInfiniteScroll(section);
                    });
                });
            });
...
(function () {
    function patch() {
        var icon = document.querySelector('a[href*="name=jellySeerr"] .MuiListItemIcon-root');
