...
public class DiscoveryService
{
    ...
    {
        AdvancedDiscoverySettings discovery = AdvancedSettingsHelper.Resolve(JellySeerrPlugin.Instance.Configuration).Discovery;
    ...
    public QueryResult<BaseItemDto> Search(string username, string query, string? language = null)
    {
        ...
        {
            _logger.LogWarning(ex, "JS • failed to search Seerr for {Query}", query);
            return EmptyResult();
        }
    }
    ...
    public QueryResult<BaseItemDto> GetDiscoverRow(string username, string jellyseerrPath, string? mediaTypeFilter = null, int startIndex = 0, int? limit = null, bool useSeerrMapping = false)
    {
        ...
        {
            _logger.LogWarning("JS • release type filters are configured but no TMDB API key set");
            return EmptyResult();
        }
        ...
        while (items.Count < targetLimit && jellyseerrPage <= maxJellyseerrPages)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogWarning(ex, useTmdbReleaseFilter
                    ? "JS • failed to fetch TMDB discover movies for path {Path}"
                    : "JS • failed to fetch Seerr path {Path}", jellyseerrPath);
                break;
            }
    ...
    {
        string scope = AdvancedSettingsHelper.Resolve(JellySeerrPlugin.Instance.Configuration)
    ...
    private JObject? FetchJellyseerrDetails(string username, PluginConfiguration config, string mediaType, int mediaId)
    {
        ...
        {
            _logger.LogWarning(ex, "JS • failed to fetch Seerr details for {MediaType}/{MediaId}", mediaType, mediaId);
            return null;
        }
    }
    ...
    private JArray GetJsonArray(string path, string username)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("JS • Seerr request failed for {Path} with status {StatusCode}", path, response.StatusCode);
                return new JArray();
            }
        ...
        {
            _logger.LogWarning(ex, "JS • failed to fetch Seerr array from {Path}", path);
            return new JArray();
        }
    }
    ...
    private JObject? FetchTmdbTrendingMoviesJson(string apiKey, PluginConfiguration config, int page, Dictionary<int, bool> releaseTypeCache)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogWarning("JS • TMDB trending request failed with status {StatusCode} for {Url}", (int)response.StatusCode, url);
            return null;
        }
    ...
    private bool MovieMatchesReleaseTypes(int movieId, IReadOnlyList<int> releaseTypes, string apiKey, Dictionary<int, bool> cache)
    {
        ...
        {
            _logger.LogDebug(ex, "JS • failed to fetch TMDB release dates for movie {MovieId}", movieId);
        }
    ...
    private JObject? FetchTmdbDiscoverJson(string url, string apiKey)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogWarning("JS • TMDB discover request failed with status {StatusCode} for {Url}", (int)response.StatusCode, url);
            return null;
        }
